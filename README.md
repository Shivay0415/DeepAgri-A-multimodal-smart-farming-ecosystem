...
Module 3 is now backed by a local disease catalog dataset.

- Dataset file: `backend/data/disease_catalog.json`
- Backend service: symptom and crop matching from the uploaded filename plus symptom hint
- Current limitation: this is still a dataset-backed matcher, not true pixel-level CNN inference yet
...
Module 5 now uses a bundled multilingual agriculture knowledge base.

- Dataset file: `backend/data/agri_knowledge_base.json`
- Backend service: keyword and context matching for English, Hindi, and Tamil responses
